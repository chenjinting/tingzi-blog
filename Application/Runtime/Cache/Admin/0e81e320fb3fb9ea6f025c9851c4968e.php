...
<div class="sidebar">
    <div class="sidebar-menu">
        ...
        <div class="showlistarticlecomment main">
            <h1>
                当前留言的文章为：  <?php if(is_array($articleidname)): $i = 0; $__LIST__ = $articleidname;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$idname): $mod = ($i % 2 );++$i; echo ($idname["title"]); ?>
            ...
            <?php if(!empty($commentlist)): ?><div class="comment-list">
                <?php if(is_array($commentlist)): $k = 0; $__LIST__ = $commentlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><div class="every-comment">
                    ...
                    <?php if($vo["status"] == 0): ?><div class="comment-operate">
                            <a class="modifybutton" href="<?php echo U('/Admin/Comment/reviewcomment',array('articleid'=>$vo['articleid'],'commentid'=>$vo['commentid']));?>">审核通过</a>
                            <a class="deletebutton" href="<?php echo U('/Admin/Comment/deletecomment',array('articleid'=>$vo['articleid'],'commentid'=>$vo['commentid']));?>">审核不通过</a>
                        </div>
                        
                    <?php else: ?>
                        <div class="comment-operate comment-delete">
                            <a class="deletebutton" href="<?php echo U('/Admin/Comment/deletecomment',array('articleid'=>$vo['articleid'],'commentid'=>$vo['commentid']));?>" onclick="return confirm('你真的要删除这条留言吗？');">删除</a>
                        </div><?php endif; ?>
